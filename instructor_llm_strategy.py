...
class InstructorLLMStrategy(LLMStrategyInterface):
    ...
    def _retry_with_backoff(self, func: Callable[[], T], max_retries: int = 3, operation_name: str = "API call") -> T:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Check if this is a rate limit error (simple check)
                is_rate_limit = "429" in error_str
                
                if attempt < max_retries:
                    delay = self._calculate_backoff_delay(attempt)
                    print(f"[RETRY] {operation_name} failed (attempt {attempt + 1}/{max_retries + 1}). Error: {error_str[:100]}...")
                    print(f"[RETRY] Waiting {delay:.1f} seconds before retry...")
                    time.sleep(delay)
                    continue
                else:
                    print(f"[ERROR] {operation_name} failed after {max_retries + 1} attempts. Final error: {error_str}")
                    break
    ...
    def get_column_suggestions(
        ...
        response = self._retry_with_backoff(
            _make_suggestions_request, 
            max_retries=3, 
            operation_name="column suggestions"
        )
        ...
        try:
            ...
            def _make_title_request():
                return self.client.chat.completions.create(
                    ...
                    temperature=0.7,
                    max_tokens=50,  # Set back to 50 as requested
                )
    ...
    def check_question_relevance(self, user_message: str) -> models.IsChatQueryRelevant:
        ...
        try:
            ...
            response = self._retry_with_backoff(
                _make_relevance_request, 
                max_retries=2,  # Lower retries for relevance check since it's less critical
                operation_name="question relevance check"
            )
    ...
    def validate_response(
        ...
        try:
            ...
            validation = self._retry_with_backoff(
                _make_validation_request, 
                max_retries=2,  # Lower retries for validation since it's less critical
                operation_name="response validation"
            )
            return validation
        except Exception:
