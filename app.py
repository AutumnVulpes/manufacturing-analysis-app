    ...
    if len(pca_cols) >= 2:
        ...
        with st.expander("PC1 Formula Details"):
            ...
            for i, col in enumerate(pca_cols):
                st.latex(fr"PC1 = PC1 + ({pca.components_[0][i]:.4f} \times \frac{{{col} - \mu_{{{col}}}}}{{\sigma_{{{{col}}}}}}")
        
        with st.expander("PC2 Formula Details"):
            st.write("PC2 = ")
            for i, col in enumerate(pca_cols):
                st.latex(fr"PC2 = PC2 + ({pca.components_[1][i]:.4f} \times \frac{{{col} - \mu_{{{col}}}}}{{\sigma_{{{{col}}}}}}")
    else:
        if exclude_cols:
            st.warning(f"Only {len(pca_cols)} column(s) available for PCA. Please exclude fewer columns to have at least two.")
        else:
